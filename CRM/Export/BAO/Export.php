...
WHERE  {$whereClause}";
   ...
   * @return array
   */
  public static function setHeaderRows($field, $headerRows, $sqlColumns, $processor, $value, $relationQuery) {
  ...
  public static function getExportStructureArrays($returnProperties, $processor, $relationQuery) {
    ...
    foreach ($returnProperties as $key => $value) {
      if ($key != 'location' || !is_array($value)) {
        ...
        list($headerRows, $sqlColumns) = self::setHeaderRows($key, $headerRows, $sqlColumns, $processor, $value, $relationQuery);
      }
      else {
        foreach ($value as $locationType => $locationFields) {
          foreach (array_keys($locationFields) as $locationFieldName) {
            ...
            list($headerRows, $sqlColumns) = self::setHeaderRows($outputFieldName, $headerRows, $sqlColumns, $processor, $value, $relationQuery);
